...
export class IntegrationsController {
  ...
  @Permission('integrations', 'write')
  saveAlertSettings(@Body() body: Omit<AlertEmailSettings, 'id'>) {
  ...
  @HttpCode(200)
  saveMyAlertPrefs(@Body() body: UserAlertEmailPrefs, @Req() req: Request) {
    const me = (req as any).user;
  ...
  @Permission('integrations', 'write')
  createReportSubscription(@Body() body: Omit<ReportEmailSubscription, 'id'>) {
  ...
  updateReportSubscription(
    ...
    @Body() body: Partial<ReportEmailSubscription>,
  ) {
